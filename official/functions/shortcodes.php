...
// Shortcode : Accordion
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
function official_accordion_shortcode($atts, $content=null) {

	return '<ul class="tt-accordion" >' . do_shortcode($content) . '</ul>';

}

function official_accordion_item_shortcode($atts, $content=null) {
	...
	return '<li class="sub-accordion"><div class="accordion-head"><div class="accordion-head-sign"></div><p>' . $title . '</p></div><div class="accordion-content">' . do_shortcode($content) .'</div></li>';
		
	
}

add_shortcode('accordion', 'official_accordion_shortcode'); 
add_shortcode('accordion-item', 'official_accordion_item_shortcode'); 
...
function official_gmap_shortcode($atts) {
	...
	$gs_class='';
	if ($grayscale=='yes'){$gs_class=' grayscale ';}
	
	$mapid = 'map' . rand();
							
	$str = '<div id="' . $mapid . '" style="width:'.$width.';height:' . $height . 'px;" class="gmap ' . $gs_class . $style . '"></div>';
	...
	if($apikey != '') 
	{
		$str_apikey = '?key=' . $apikey ;
	}

	$str .= '
	<script type="text/javascript" src="//maps.google.com/maps/api/js'.$str_apikey.'"></script>
	<script type="text/javascript">
	function mapfucntion_' . $mapid . '() {
		...
		if ($color!=''){
			$str .= 'var styles = [{stylers: [{ hue: "'.$color.'" },{ saturation: -20 }]}];';
			$str .= 'var styledMap = new google.maps.StyledMapType(styles,{name: "Styled Map"});';
			$str .=  $mapid . '.mapTypes.set("map_style", styledMap);  ';
			$str .=  $mapid . '.setMapTypeId("map_style");';
		}
		...
		if($address != '')
		{
			...
			geocoder_' . $mapid . '.geocode( { \'address\': address}, function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					if ($markerimage !='')
					{
						if(is_numeric($markerimage)){$markerimage = wp_get_attachment_url( $image );}
		...
		google.maps.event.addDomListener(window, "load", mapfucntion_' . $mapid . ');
...
function official_item_shortcode( $atts, $content = null ) {
	...
	$out = '<li><i class="icon-'.$thdglkr_list_icon.'"></i><p>'. do_shortcode($content) . '</p></li>';
    return $out;
}
...
extract( shortcode_atts( array(
	  ...
	  $icons = array(
		'facebook','twitter','google_plus','linkedin','dribbble','rss','flickr','pinterest','instagram','skype','tumblr','youtube','xing','dropbox','stackexchange','bitbucket','weibo','github','foursquare'
	  );
...
add_shortcode( 'tt_video', 'official_video_shortcode' );
...
extract( shortcode_atts( array(
      ...
      if($image == '') {
		...
		if(is_numeric($image)){$imgsrc = wp_get_attachment_url( $image );}else{$imgsrc= $image;}
    	$return = "<div class='member-img hover-fx zoom'><img src='".$imgsrc."' alt='".$name."' /></div>";
      }
      ...
      if( $twitter != '' || $facebook != '' || $linkedin != '' || $google_plus != '' || $dribbble != '' || $flickr != '' || $skype != '' || $instagram != '' || $email != '' ){
		  ...
		  if($dribbble != '') {
	    	$return9 = '<li><a href="' .$dribbble. '" target="_blank" title="Dribbble" class="toptip"><i class="icon-dribbble"></i></a></li>';
	      }
	      else{
		      $return9 = ''; 
	      }
