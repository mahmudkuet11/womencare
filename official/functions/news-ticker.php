...
if ($thdglkr_nt_order==''){$thdglkr_nt_order='DESC';}
	...
	<script>
		...
		jQuery(document).ready(function($) {	
			// News Ticker /////////////////////////////
			try {
				jQuery("ul#<?php echo esc_js($ticker_id); ?>").liScroll({travelocity: 0.08});
				jQuery("ul#<?php echo esc_js($ticker_id); ?> li").show();
			} catch(e){}
		});	
	</script>
	...
	<?php 
		...
		if ($thdglkr_nt_color != ''){$title_color = ' style="color:'.$thdglkr_nt_color.';" ';}
				...
				while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
					...
					<li style="display:none"><span><?php the_time('d M'); ?></span><a href="<?php the_permalink() ?>" ><?php the_title(); ?></a></li>
