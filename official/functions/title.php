...
if(is_single()){$title_bar=_option('blog_title_type','cpmb_title');}
	...
	switch ($title_bar) {
		...
		case 'cpmb_image';
			if (!$is_woo){
				...
				$featured_img = _option('woo_featured_img','');
				if($featured_img!=''){
				?>
                    <!-- Titlebar Image -->   
                    <div class="image_titlebar">
                        <img class="titlebar" src="<?php echo esc_url($featured_img); ?>" alt="<?php the_title(); ?>" /> 
                    </div>             
				<?php 
				}
			}
