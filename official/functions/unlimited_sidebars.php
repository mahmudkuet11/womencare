...
Hot fixed by Tohid Golkar (ThemeTor) for Security issue (XSS vulnerability) on 14 Dec 2016 
*/
...
class sidebar_generator {
	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'name'=>$sidebar,
					'id' => 'thdglkr-custom-sidebar-'.strtolower($sidebar_class),
		...
		?>
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					mysack.setVar( 'sidebar_generator_nonce', '<?php echo wp_create_nonce('sidebar_generator_nonce'); ?>' );
	...
	public static function add_sidebar(){
		//Hot fixed by Tohid Golkar (ThemeTor) for Security issue (XSS vulnerability) on 14 Dec 2016
		$sidebar_generator_nonce = $_POST['sidebar_generator_nonce'];
		if (! wp_verify_nonce($sidebar_generator_nonce, 'sidebar_generator_nonce') ) die("Security check");
		$sidebars = sidebar_generator::get_sidebars();
					...
					//var_dump($wp_registered_sidebars);
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator_replacement[<?php echo $i?>]">
								<option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>>Default</option>
							<?php
