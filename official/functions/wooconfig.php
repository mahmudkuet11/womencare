...
if ( ! function_exists( 'woocommerce_output_upsells' ) ) {
	function woocommerce_output_upsells() {
	    woocommerce_upsell_display( _option('woo_col',3),_option('woo_col',3) );
	}
}

function smartco_filter_woocommerce_cross_sells_columns( $columns ){
    return _option('woo_col',3);
};   
add_filter( 'woocommerce_cross_sells_columns', 'smartco_filter_woocommerce_cross_sells_columns', 10, 1 );
...
$form = '<form role="search" method="get" id="searchform" action="' . esc_url( home_url( '/' ) ) . '">
			<div>
			<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . __( 'Search...', 'official' ) . '" />
